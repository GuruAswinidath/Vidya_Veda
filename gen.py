...
#     title = st.text_input("Enter the Subject/Topic: ")
...
#     st.success("PPT content created")
...
st.title("PPT Slide Generator with JSON Output")

# Input for topic
title = st.text_input("Enter the Subject/Topic:")

# Function to run LLM chain and process JSON output
def generate_ppt_content(title):
    # Define the prompt template
    prompt = PromptTemplate(
        input_variables=["title"],
        template=prompt_template
    ...
    # Run the chain with the input title
    output = chain.run(title=title)

    # # Remove any code block markers from the output
    # output = output.replace("```json", "").replace("```", "")

    # # Clean up the JSON string
    # sanitized_output = re.sub(r",\s*]", "]", output)
    # sanitized_output = re.sub(r",\s*}", "}", sanitized_output)

    out  = output.replace("```json","")
    out  = out.replace("```","")
    # Attempt to parse the sanitized output as JSON
    try:
        json_output = json.loads(out)
        with open('output.json', 'w') as json_file:
            json.dump(json_output, json_file, indent=4)
        st.success("JSON output successfully saved to 'output.json'.")
        st.json(json_output)  # Display JSON in Streamlit




    except json.JSONDecodeError as e:
        st.error("Failed to decode output as JSON.")
        st.write("Error:", e)
        st.text("Sanitized Output for Debugging:\n")
        # st.text(sanitized_output)
...
# Run content generation when the button is clicked
if st.button("Generate PPT Content"):
    if title:
        generate_ppt_content(title)
